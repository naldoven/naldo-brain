...
function fmtDate(s: string | null | undefined): string {
  if (!s) return "—";
  // Plaid date is "YYYY-MM-DD" — render in user tz
    ...
    <div>
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            ...
            <div className="text-sm text-zinc-400 mt-1">
              {(snapshot.debtPctPaid * 100).toFixed(1)}% paid · {fmtUsd(snapshot.debtTotal, { compact: true })} remaining
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-4">
        ...
        <div className="glass-strong rounded-2xl p-5">
          ...
          {accounts.length === 0 ? (
            <p className="text-xs text-zinc-500">No accounts yet.</p>
          ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-zinc-500 truncate">
                        {item?.institution_name ?? "—"} · {a.subtype ?? a.type ?? "account"}
                      </div>
                    </div>
      ...
      {items.length > 0 && (
        <div className="mt-4 text-[11px] text-zinc-500 flex flex-wrap gap-3">
          {items.map((it) => (
            <span key={it.id} className="inline-flex items-center gap-1.5">
              ...
              />
              {it.institution_name ?? "Connection"} · {it.status}
              {it.last_synced_at
